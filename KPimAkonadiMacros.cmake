    ...
    function(add_akonadi_isolated_test_impl)
        ...
        function(_defineTest name backend)
            ...
            if(
                (
                    NOT DEFINED
                        AKONADI_RUN_${backend}_ISOLATED_TESTS
                    OR AKONADI_RUN_${backend}_ISOLATED_TESTS
                )
                AND (
                    NOT DEFINED
                        AKONADI_RUN_ISOLATED_TESTS
                    OR AKONADI_RUN_ISOLATED_TESTS
                )
            )
                ...
                if(${backendsLen} EQUAL 0 OR ${enableBackend} GREATER -1)
                    ...
                    add_test(
                        NAME ${_test_name}
                        COMMAND
                            ${_testrunner} -c "${configFile}" -b ${lcbackend} ${_executable} ${extraOptions}
                    )
...
function(kcfg_generate_dbus_interface _kcfg _name)
    ...
    add_custom_command(
        ...
        COMMAND
            ${XSLTPROC_EXECUTABLE} --output ${_name}.xml --stringparam interfaceName ${_name} ${xsl_relpath}
            ${kcfg_relpath}
        DEPENDS
            ${xsl_path}
            ${_kcfg}
