...
Generates a check to test the installation of a library for the self-containedness
...
can be invoked after installation to check if the installed library artifacts
are self-contained when used from a consumer. TODO: specify type pf checks.
...
function(ecm_add_installed_library_check _target)
    ...
    if(ARG_EXTRA_DEPENDENCIES)
        ...
        math(EXPR _is_even_number "${_deps_count} % 2")
        if(NOT _is_even_number EQUAL 0)
            message(FATAL_ERROR "EXTRA_DEPENDENCIES arguments has uneven number: ${ARG_EXTRA_DEPENDENCIES}")
        endif()
        set(_i 0)
        while(${_i} LESS ${_deps_count})
            ...
            # TODO: check if version, if not, optionally skip it
            math(EXPR _i "${_i} + 1")
            set(_extra_find_packages_statements
                "find_package(${_extra_package_name} ${_extra_package_version} REQUIRED)\n"
            )
        endwhile()
    endif()
