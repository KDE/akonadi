...
void StorageJanitor::findOrphanedItems()
{
    ...
    query.prepare(u"UPDATE %1 SET %2 = :col WHERE %3 NOT IN (SELECT DISTINCT %4 from %5)"_s.args(PimItem::tableName(), PimItem::collectionIdColumn(), PimItem::collectionIdColumn(), Collection::idColumn(), Collection::tableName()));
