...
#define AKONADI_AGENTCONFIG_FACTORY(FactoryName, metadata, ClassName) \
    class FactoryName : public AgentConfigurationFactoryBase { \
        Q_OBJECT \
        Q_PLUGIN_METADATA(IID "org.freedesktop.Akonadi.AgentConfig" FILE metadata) \
    public: \
        FactoryName(QObject *parent = nullptr): QObject(parent) {} \
        AgentConfigurationBase *create(KSharedConfigPtr config, QWidget *parent, const QVariantList &args) override { \
