...
Task<Item::List> Storage::fetchItemsFromCollection(const Collection &collection, const ItemFetchOptions &options, Session *session)
{
    return asyncJob(createJob<ItemFetchJob>(), &ItemFetchJob::items, session, options, collection);
}

Task<Item::List> Storage::fetchItems(const Item::List &items, const ItemFetchOptions &options, Session *session)
{
...
Task<Item> Storage::createItem(const Item &item, const Collection &collection, ItemCreateFlags flags, Session *session)
{
    return asyncJob(
        [](Session *session, ItemCreateFlags flags, const Item &item, const Collection &collection) {
            ...
            if (flags & ItemCreateFlag::RID) {
                options |= ItemCreateJob::RID;
            }
            if (flags & ItemCreateFlag::GID) {
                options |= ItemCreateJob::GID;
            }
...
Task<Item::List> Storage::updateItems(const Item::List &items, ItemModifyFlags flags, Session *session)
{
...
Task<void> Storage::moveItemsFromCollection(const Item::List &items, const Collection &source, const Collection &destination, Session *session)
{
...
Task<void> Storage::copyItems(const Item::List &items, const Collection &destination, Session *session)
{
...
Task<Item::List> Storage::searchItems(const SearchQuery &searchQuery, const ItemSearchOptions &options, Session *session)
{
    return asyncJob(
...
Task<Collection> Storage::fetchCollection(const Collection &collection, const CollectionFetchOptions &options, Session *session)
{
    return asyncJob(
...
Task<Collection::List> Storage::fetchCollectionsRecursive(const Collection &root, const CollectionFetchOptions &options, Session *session)
{
    return asyncJob(createJob<CollectionFetchJob>(), &CollectionFetchJob::collections, session, options, root, CollectionFetchJob::Recursive);
}

Task<Collection::List> Storage::fetchCollectionsRecursive(const Collection::List &collections, const CollectionFetchOptions &options, Session *session)
{
    return asyncJob(createJob<CollectionFetchJob>(), &CollectionFetchJob::collections, session, options, collections, CollectionFetchJob::Recursive);
}

Task<Collection::List> Storage::fetchCollections(const Collection::List &collections, const CollectionFetchOptions &options, Session *session)
{
    return asyncJob(createJob<CollectionFetchJob>(), &CollectionFetchJob::collections, session, options, collections);
}

Task<Collection::List> Storage::fetchSubcollections(const Collection &base, const CollectionFetchOptions &options, Session *session)
{
    return asyncJob(createJob<CollectionFetchJob>(), &CollectionFetchJob::collections, session, options, base, CollectionFetchJob::FirstLevel);
}
...
Task<void> Storage::moveCollection(const Collection &collection, const Collection &destination, Session *session)
{
    return asyncJob(createJob<CollectionMoveJob>(), session, collection, destination);
}

Task<void> Storage::copyCollection(const Collection &collection, const Collection &destination, Session *session)
{
    return asyncJob(createJob<CollectionCopyJob>(), session, collection, destination);
}

Task<CollectionStatistics> Storage::fetchCollectionStatistics(const Collection &collection, Session *session)
{
...
Task<Collection> Storage::createSearchCollection(const QString &name, const SearchQuery &searchQuery, const ItemSearchOptions &options, Session *session)
{
    return asyncJob(
        ...
        &SearchCreateJob::createdCollection,
        session,
        options,
        name,
        searchQuery);
