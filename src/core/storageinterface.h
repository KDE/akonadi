...
public:
     ...
     */
    virtual Task<Item::List> fetchItemsFromCollection(const Collection &collection, const ItemFetchOptions &options,
                                                      Session *session = nullptr) = 0;
     ...
     */
    virtual Task<Item::List> fetchItems(const Item::List &items, const ItemFetchOptions &options,
                                        Session *session = nullptr) = 0;
     ...
     */
    virtual Task<Item::List> fetchItemsForTag(const Tag &tag, const ItemFetchOptions &options,
                                              Session *session = nullptr) = 0;
     ...
     */
    virtual Task<Item> fetchItem(const Item &item, const ItemFetchOptions &options,
                                 Session *session = nullptr) = 0;
     ...
     */
    virtual Task<Item> createItem(const Item &item, const Collection &collection,
                                  ItemCreateFlags flags = ItemCreateFlag::Default,
                                  Session *session = nullptr) = 0;
     ...
     * @see updateItems()
     */
    virtual Task<Item> updateItem(const Item &item, ItemModifyFlags flags = ItemModifyFlag::Default,
                                  Session *session = nullptr) = 0;
     ...
     * @see updateItem()
     */
    virtual Task<Item::List> updateItems(const Item::List &items, ItemModifyFlags flags = ItemModifyFlag::Default,
                                         Session *session = nullptr) = 0;
     ...
     */
    virtual Task<void> moveItem(const Item &item, const Collection &destination,
                                Session *session = nullptr) = 0;
     ...
     */
    virtual Task<void> moveItems(const Item::List &items, const Collection &destination,
                                 Session *session = nullptr) = 0;
     ...
     */
    virtual Task<void> moveItemsFromCollection(const Item::List &items, const Collection &source,
                                               const Collection &destination, Session *session = nullptr) = 0;
     ...
     * @see copyItem()
     */
    virtual Task<void> copyItems(const Item::List &items, const Collection &destination,
                                        Session *session = nullptr) = 0;
     ...
     * @see unlinkItems()
     */
    virtual Task<void> linkItems(const Item::List &items, const Collection &destination,
                                 Session *session = nullptr) = 0;
     ...
     * @see linkItems()
     */
    virtual Task<void> unlinkItems(const Item::List &items, const Collection &collection,
                                   Session *session = nullptr) = 0;
     ...
     */
    virtual Task<Item::List> searchItems(const SearchQuery &searchQuery, const ItemSearchOptions &options,
                                         Session *session = nullptr) = 0;
     ...
     */
    virtual Task<Collection> fetchCollection(const Collection &collection, const CollectionFetchOptions &options,
                                             Session *session = nullptr) = 0;
     ...
     */
    virtual Task<Collection::List> fetchCollectionsRecursive(const Collection &root, const CollectionFetchOptions &options,
                                                             Session *session = nullptr) = 0;
     ...
     */
    AKONADICORE_EXPORT
    virtual Task<Collection::List> fetchCollectionsRecursive(const Collection::List &collections, const CollectionFetchOptions &options = {},
                                                             Session *session = nullptr) = 0;
     ...
     */
    virtual Task<Collection::List> fetchCollections(const Collection::List &collections, const CollectionFetchOptions &options,
                                                    Session *session = nullptr) = 0;
     ...
     */
    virtual Task<Collection::List> fetchSubcollections(const Collection &base, const CollectionFetchOptions &options,
                                                       Session *session = nullptr) = 0;
     ...
     */
    virtual Task<void> moveCollection(const Collection &collection, const Collection &destination,
                                             Session *session = nullptr) = 0;
     ...
     */
    virtual Task<void> copyCollection(const Collection &collection, const Collection &destination,
                                      Session *session = nullptr) = 0;
     ...
     */
    virtual Task<CollectionStatistics> fetchCollectionStatistics(const Collection &collection,
                                                                 Session *session = nullptr) = 0;
     ...
     * @see fetchTag()
     */
    virtual Task<Tag::List> fetchTags(const Tag::List &tags, const TagFetchOptions &options,
                                      Session *session = nullptr) = 0;
     ...
     * @see fetchRelation()
     */
    virtual Task<Relation::List> fetchRelationsOfTypes(const QVector<QByteArray> &types,
                                                       Session *session = nullptr) = 0;
     ...
     *        collection.
     */
    virtual Task<Collection> createSearchCollection(const QString &name, const SearchQuery &searchQuery,
                                                    const ItemSearchOptions &options,
                                                    Session *session = nullptr) = 0;
