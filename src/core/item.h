...
/*!
 * \brief Represents a PIM item stored in Akonadi storage.
 *
...
public:
    ...
    /*!
     * Creates a new item with the given unique \a id.
     */
    explicit Item(Id id);
     ...
     *
     * \a mimeType The mime type of the item.
     */
    explicit Item(const QString &mimeType);

    /*!
     * Creates a new item from an \a other item.
     */
    ...
    /*!
     * Creates an item from the given \a url.
     */
    static Item fromUrl(const QUrl &url);

    /*!
     * Sets the unique \a identifier of the item.
     */
    ...
    /*!
     * Sets the remote \a id of the item.
     */
    ...
    /*!
     * Sets the remote \a revision of the item.
     * \a revision the item's remote revision
    ...
    /*!
     * Returns whether this item's id equals the id of the \a other item.
     */
    bool operator==(const Item &other) const;

    /*!
     * Returns whether the item's id does not equal the id of the \a other item.
     */
    bool operator!=(const Item &other) const;

    /*!
     * Assigns the \a other to this item and returns a reference to this item.
    ...
    /*!
     * Returns \\ true if the item has an attribute of the given type \a name,
     * false otherwise.
     */
     ...
     * \a datetime the modification time to set
     * \note Do not modify this value from within an application,
    ...
    /*!
     * Returns whether the flag with the given \a name is
     * set in the item.
     */
    bool hasFlag(const QByteArray &name) const;

    /*!
     * Sets the flag with the given \a name in the item.
     */
    void setFlag(const QByteArray &name);

    /*!
     * Removes the flag with the given \a name from the item.
     */
    void clearFlag(const QByteArray &name);

    /*!
     * Overwrites all flags of the item by the given \a flags.
     */
    ...
    /*!
     * Sets the \a revision number of the item.
     * \a revision the revision number to set
     * \note Do not modify this value from within an application,
     * it is updated automatically by the revision checking functions.
     */
     ...
     * \a size the size of the item in bytes
     * \since 4.2
     */
    ...
    /*!
     * Sets the mime type of the item to \a mimeType.
     */
    ...
    /*!
     * Sets the virtual \a collections that this item is linked into.
     *
     * \note Note that changing this value makes no effect on what collections
     ...
     *
     * \note This value is populated only when this item was retrieved by
     ...
     *
     * \sa setPayload(), setPayloadFromData()
     * \since 5.6
     */
     ...
     *
     * \a p The payload object. Must be copyable and must not be a pointer,
     ...
     * of the actual parts in cache, as it contains only the requested parts. See \sa ItemFetchJob and
     * \sa ItemFetchScope
     *
    ...
    /*!
     * Applies the parts of Item \a other to this item.
     ...
     * of the payload.
     * \a other the item to get values from
     * \since 4.4
     */
...
private:
     ...
     * Returns \\ true if a type exists or could be created through conversion, \\ false otherwise.
     */
     ...
     * \a collectionId the unique identifier of the collection where this item is stored in.
     * \since 4.3
     */
