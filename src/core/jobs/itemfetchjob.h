...
public:
     ...
     *
     * \a collection The parent collection to fetch all items from.
    ...
    /*!
     * Creates a new item fetch job that retrieves all items tagged with specified \a tag.
     *
     * \a tag The tag to fetch all items from.
     * \a parent The parent object.
     *
     * \since 4.14
     */
...
ote The items are invalid before the result(KJob*)
     ...
     *
     * \internal
     * \sa ResourceSelectJob (for internal use only)
     * @endinternal
     */
     ...
     *
     * \a limit the maximum number of items to retrieve.
     * The default value for \a limit is -1, indicating no limit.
     * \a start specifies the offset of the first item to retrieve.
     * \a order specifies whether items will be fetched
